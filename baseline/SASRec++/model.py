...
class Model(nn.Module):
    def __init__(self, itemnum, ItemMeanFeatures, MetaFeatures, args):
        ...
        self.target_layer_norm = nn.LayerNorm(self.hidden_units, eps=1e-8)
    ...
    def seq_self_attention_layer(self, targetSeq):
        ...
        target_meta_feat = self.feat_emb(torch.LongTensor(targetSeq).to(self.dev))
        ...
        # targetFeat -> SASRec
        target_meta_feat = target_meta_feat * (self.item_emb.embedding_dim ** 0.5)
        target_meta_feat = target_meta_feat + self.pos_emb(torch.LongTensor(positions).to(self.dev))
        target_meta_feat = self.multihead_attention_layers(target_meta_feat, target_timeline_mask)

        target_seq_feat = torch.cat((targetSeqs, target_meta_feat), dim=-1)
        target_seq_feat = self.last_layernorm(self.seqFeatLinear(target_seq_feat))

        return target_seq_feat
    ...
    def forward(self, targetSeq, pos, neg):
        ...
        pos_meta_feat = self.feat_emb(torch.LongTensor(pos).to(self.dev))
        pos_seq_feat = torch.cat((pos_item_in, pos_meta_feat), dim=-1)
        pos_emb = self.seqFeatLinear(pos_seq_feat)

        neg_meta_feat = self.feat_emb(torch.LongTensor(neg).to(self.dev))
        neg_seq_feat = torch.cat((neg_item_in, neg_meta_feat), dim=-1)
        neg_emb = self.seqFeatLinear(neg_seq_feat)

        pos_logits = (seq_sasrec * pos_emb).sum(dim=-1)
        neg_logits = (seq_sasrec * neg_emb).sum(dim=-1)
    ...
    def predict(self, targetSeq, test_item):
        ...
        test_meta_feat = self.feat_emb(torch.LongTensor(test_item).to(self.dev))
        test_item_emb = torch.cat((test_item_in, test_meta_feat), dim=-1)
        test_item_emb = self.seqFeatLinear(test_item_emb)
