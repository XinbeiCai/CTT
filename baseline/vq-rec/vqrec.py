...
class VQRec(SequentialRecommender):
    ...
    def calculate_item_emb(self):
        if self.index_assignment_flag:
            pq_code_emb = F.embedding(self.pq_codes, self.reassigned_code_embedding, padding_idx=0) #(91170,32,300)
            pq_code_emb = torch.mean(pq_code_emb, dim=-2)#(91170,300)
        else:
    ...
    def full_sort_predict(self, interaction):
        ...
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        seq_output = self.forward(item_seq, item_seq_len) #(1024,300)
        test_items_emb = self.calculate_item_emb() #(91170,300)
    ...
    def predict(self, interaction):
        item_seq = interaction[self.ITEM_SEQ] #(1010,50)
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        seq_output = self.forward(item_seq, item_seq_len) #(1010, 300)
        test_item = interaction[self.ITEM_ID]
        pq_code_seq = self.pq_codes[test_item] #(1010,50,32)
        if self.index_assignment_flag:
            # (1010,50,32) * (8224,300) = (1010,50,300)
            test_items_emb = F.embedding(pq_code_seq, self.reassigned_code_embedding, padding_idx=0)
            test_items_emb = torch.mean(test_items_emb, dim=-2) #(1010,300)
        else:
            test_items_emb = self.pq_code_embedding(pq_code_seq).mean(dim=-2)

        scores = torch.mul(seq_output, test_items_emb).sum(dim=1)
        return scores
