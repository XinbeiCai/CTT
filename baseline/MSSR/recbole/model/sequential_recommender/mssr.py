...
#                    print('total_loss:{}\titem_loss:{}\tattribute_{}_loss:{}'.format(total_loss, loss, self.selected_features[0], attribute_loss))
    ...
    def predict(self, interaction):
        ...
        test_item = interaction[self.ITEM_ID]
        test_item_emb = self.item_embedding(test_item)
        feature_emb = self.get_cd_fea_emb(test_item)
        if self.config['pred'] == 'dot':
            # score_ii = torch.matmul(seq_output, test_item_emb.transpose(0, 1))
            # score_cc = torch.matmul(seq_output_attr, feature_emb.transpose(0, 1))
            score_ii = torch.mul(seq_output, test_item_emb).sum(dim=1)
            score_cc = torch.mul(seq_output_attr, feature_emb).sum(dim=1)

        if self.logit_num == 4:
            score_ic = torch.matmul(seq_output, self.linear_w_ci(feature_emb).transpose(0, 1))
            score_ci = torch.matmul(seq_output_attr, self.linear_w_ic(test_item_emb).transpose(0, 1))
