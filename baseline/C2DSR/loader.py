...
class DataLoader(object):
    ...
    """
    def __init__(self, filename, batch_size, opt, used_data, evaluation):
        self.batch_size = batch_size
        self.opt = opt
        self.eval = evaluation
        self.filename = filename
        # self.source_neg_sample = source_neg_sample
        # self.target_neg_sample = target_neg_sample
        # target_domain, source_domain = opt['target_domain'], opt['source_domain']
        # itemnum_target_domain, itemnum_source_domain, train_data, valid_data, test_data = self.data_partition(target_domain, source_domain)
        # ************* item_id *****************
        ...
        if evaluation < 0:
            self.train_data = used_data
            data = self.preprocess()
        elif evaluation == 2:
            self.test_data = used_data
    ...
    def preprocess_for_predict(self):
        ...
        for d in self.test_data: # the pad is needed! but to be careful.
            ...
            elif len(d[0]) > max_len:
                position = position[-max_len:]
                x_position = x_position[-max_len:]
                y_position = y_position[-max_len:]
            ...
            negative_sample = []
            # if d[1]: negative_sample = self.target_neg_sample
            # else: negative_sample = self.source_neg_sample
            for i in range(100):
                while True:
    ...
    def preprocess(self):
        ...
        for d in self.train_data: # the pad is needed! but to be careful.
            ...
            if len(d) < max_len:
                ...
                ground = ground[-max_len:]
                share_x_ground = share_x_ground[-max_len:]
                share_y_ground = share_y_ground[-max_len:]
                x_ground = x_ground[-max_len:]
                y_ground = y_ground[-max_len:]

                ground_mask = ground_mask[-max_len:]
                share_x_ground_mask = share_x_ground_mask[-max_len:]
                share_y_ground_mask = share_y_ground_mask[-max_len:]
                x_ground_mask = x_ground_mask[-max_len:]
                y_ground_mask = y_ground_mask[-max_len:]

                corru_x = corru_x[-max_len:]
                corru_y = corru_y[-max_len:]
                xd = xd[-max_len:]
                yd = yd[-max_len:]
                d = d[-max_len:]
            else:
