...
class C2DSR(torch.nn.Module):
    def __init__(self, opt, adj, adj_single, ItemMeanFeatures, MetaFeatures):
        ...
        self.review_emb = torch.tensor(ItemMeanFeatures, dtype=torch.float32, device=self.opt["device"])
        self.meta_emb = torch.tensor(MetaFeatures, dtype=torch.float32, device=self.opt["device"])
        ...
        self.feat_target_Linear = nn.Linear(self.opt["embedding_size"], self.opt["hidden_units"])
        self.target_text_layer_norm = nn.LayerNorm(self.opt["hidden_units"], eps=self.opt["eps"])
        self.target_text_embed_dropout = nn.Dropout(self.opt["dropout"])

        self.feat_source_Linear = nn.Linear(self.opt["embedding_size"], self.opt["hidden_units"])
        self.source_text_layer_norm = nn.LayerNorm(self.opt["hidden_units"], eps=self.opt["eps"])
        self.source_text_embed_dropout = nn.Dropout(self.opt["dropout"])

        self.feat_Linear = nn.Linear(self.opt["embedding_size"], self.opt["hidden_units"])
        self.text_layer_norm = nn.LayerNorm(self.opt["hidden_units"], eps=self.opt["eps"])
        self.text_embed_dropout = nn.Dropout(self.opt["dropout"])

        self.feat_Linear_false = nn.Linear(self.opt["embedding_size"], self.opt["hidden_units"])
        self.text_layer_norm_false = nn.LayerNorm(self.opt["hidden_units"], eps=self.opt["eps"])
        self.text_embed_dropout_false = nn.Dropout(self.opt["dropout"])

        self.item_feat_linear = nn.Linear(2 * self.opt["hidden_units"], self.opt["hidden_units"])
        self.item_feat_linear_X = nn.Linear(2 * self.opt["hidden_units"], self.opt["hidden_units"])
        self.item_feat_linear_Y = nn.Linear(2 * self.opt["hidden_units"], self.opt["hidden_units"])
        self.item_feat_linear_false = nn.Linear(2 * self.opt["hidden_units"], self.opt["hidden_units"])
    ...
    def feat_emb(self, input_seq):
        ...
        seq_meta = self.meta_emb[input_seq]
        seq_feat = seq_review + seq_meta
        seq_feat = self.feat_Linear(seq_feat)
        seq_feat = self.text_layer_norm(self.text_embed_dropout(seq_feat))
        return seq_feat

    def feat_emb_false(self, input_seq):
        seq_review = self.review_emb[input_seq]
        seq_meta = self.meta_emb[input_seq]
        seq_feat = seq_review + seq_meta
        seq_feat = self.feat_Linear_false(seq_feat)
        seq_feat = self.text_layer_norm_false(self.text_embed_dropout_false(seq_feat))
        return seq_feat
    ...
    def forward(self, o_seqs, x_seqs, y_seqs, position, x_position, y_position):
        ...
        # seqs = self.item_feat_linear(torch.cat((seqs, self.feat_emb(o_seqs)), dim=-1))
        ...
        # seqs = self.item_feat_linear_X(torch.cat((seqs, self.feat_emb_X(x_seqs)), dim=-1))
        seqs *= self.item_emb.embedding_dim ** 0.5
        ...
        # seqs = self.item_feat_linear_Y(torch.cat((seqs, self.feat_emb_Y(y_seqs)), dim=-1))
        seqs *= self.item_emb.embedding_dim ** 0.5
    ...
    def false_forward(self, false_seqs, position):
        ...
        # seqs = self.item_feat_linear_false(torch.cat((seqs, self.feat_emb_false(false_seqs)), dim=-1))
        seqs *= self.item_emb.embedding_dim ** 0.5
