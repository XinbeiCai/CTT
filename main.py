...
log_file = os.path.join(dir, 'log_CTT_%s_%s.txt' % (args.target_domain, args.alpha))
...
def train_model(args):
    ...
    [user_train_target_domain, user_valid_target_domain, user_test_target_domain,
     ...
     usernum, itemnum_target_domain, itemnum_source_domain, user_neg_target_domain, ItemMeanFeatures, MetaFeatures] = dataset
    ...
    sampler = WarpSampler(user_train_target_domain, user_train_source_domain, usernum, itemnum_target_domain,
                          batch_size=args.batch_size, maxlen=args.maxlen, n_workers=3)

    model = Model(itemnum_target_domain, itemnum_target_domain + itemnum_source_domain, ItemMeanFeatures, MetaFeatures, args).to(args.device)
    ...
    with tqdm(total=args.num_epochs * num_batch, desc=f"Training", leave=False, ncols=100) as pbar:
        for epoch in range(epoch_start_idx, args.num_epochs + 1):
            ...
            for step in range(num_batch):
                ...
                [u, seq_target, seq_source, pos, neg] = [np.array(result) for result in sampler.next_batch()]
                pos_logits_sasrec, neg_logits_sasrec, con_loss = model(seq_target, seq_source, pos, neg)
            ...
            if epoch % args.eva_interval == 0:
                ...
                if epoch == args.num_epochs and args.save_model == 1:
                    fname = 'CTT.epoch={}.lr={}.layer={}.head={}.hidden={}.maxlen={}.alpha={}.pth'
