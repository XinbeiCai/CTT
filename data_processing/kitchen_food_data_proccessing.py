...
parser.add_argument('-x', default='us_Electronics', help='category')
parser.add_argument('-y', default='us_Cell_Phones_and_Accessories', help='category')
...
def raw_process(category_name):
    ...
    })[['user_id', 'asin', 'reviews', 'timestamp']]
...
def whole_process(df1, df2):
    user = pd.Series(list(set(df1['user_id']).intersection(set(df2['user_id']))))
    print("same user: ", len(user))
    items = pd.Series(list(set(df1['asin']).intersection(set(df2['asin']))))
    print("same items: ", len(items))
    print(items)

    df4 = pd.concat([df1, df2], keys=['x', 'y', 'z'])
...
if __name__ == '__main__':
    ...
    df3 = whole_process(df1, df2)
    domain_process(df3, args.x)
    domain_process(df3, args.y)
